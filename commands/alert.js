    ...
    for (var team of teams) {
      if (currentGame.number > gameNumber - 2) {
        message.guild.channels.cache.get(team[1]).send(
          `Hello, ${team[0]}! The final games will happen <t:${currentTime / 1000}:R>. Are your players ready?`
        );
      } else if (["Duo", "Duo +"].includes(currentType)) {
        message.guild.channels.cache
          .get(team[1])
          .send(
            `Hello, ${team[0]}! The game will happen <t:${currentTime / 1000}:R>. Are your player and coach ready?`
          );
      } else {
        message.guild.channels.cache
          .get(team[1])
          .send(
            `Hello, ${team[0]}! The game will happen <t:${currentTime / 1000}:R>. Is your player ready?`
          );
      }
    }
    coolDown = true;
